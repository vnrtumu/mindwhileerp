...
import {
    IconArrowLeft, IconCheck, IconRoute, IconMapPin,
    IconBus, IconUser, IconHash, IconRoad, IconPlus
} from '@tabler/icons-react';

const FIELDS = [
    { key: 'routeNo', label: 'Route Number', type: 'text', required: true, col: 1 },
    { key: 'routeName', label: 'Route Name', type: 'text', required: true, col: 1 },
    { key: 'startPoint', label: 'Start Point', type: 'text', required: false, col: 1 },
    { key: 'endPoint', label: 'End Point', type: 'text', required: false, col: 1 },
    { key: 'stops', label: 'Number of Stops', type: 'number', required: false, col: 1 },
    { key: 'distance', label: 'Distance (km)', type: 'text', required: false, col: 1 },
    { key: 'vehicle', label: 'Assigned Vehicle', type: 'text', required: false, col: 1 },
    { key: 'driver', label: 'Driver Name', type: 'text', required: false, col: 1 },
];
...
export default function AddRoute() {
    ...
    const handleSave = () => {
        ...
        setSaving(true);
        setTimeout(() => {
            const existing = JSON.parse(localStorage.getItem('routes') || '[]');
            const newRoute = { ...form, id: Date.now(), stops: Number(form.stops) || 0 };
            localStorage.setItem('routes', JSON.stringify([...existing, newRoute]));
            setSaving(false);
            setSaved(true);
    ...
    return (
        <div className="student-list-page">
            <style>{`
                ...
                .ar-hdr  { animation: slideLeft .3s ease both; }
                .ar-side { animation: slideLeft .35s ease .05s both; }
                .ar-form { animation: popIn .35s ease .08s both; }
                .ar-f0   { animation: slideUp .3s ease .12s both; }
                .ar-f1   { animation: slideUp .3s ease .16s both; }
                .ar-f2   { animation: slideUp .3s ease .20s both; }
                .ar-f3   { animation: slideUp .3s ease .24s both; }
                .ar-f4   { animation: slideUp .3s ease .28s both; }
                .ar-f5   { animation: slideUp .3s ease .32s both; }
                .ar-f6   { animation: slideUp .3s ease .36s both; }
                .ar-f7   { animation: slideUp .3s ease .40s both; }
                .ar-fst  { animation: slideUp .3s ease .44s both; }
                .ar-foot { animation: slideUp .3s ease .48s both; }
            ...
            <div className="container">
                ...
                {/* Header */}
                <div className="page-header !mb-6 ar-hdr">
                        ...
                        <div className="back-button-wrapper">
                            ...
                            <div>
                                ...
                                <nav className="breadcrumb">
                                    <span>Transport</span> / <span>Routes</span> / <span className="current">Add</span>
                                </nav>
                            </div>
                        </div>
                    </div>
                ...
                <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                    ...
                    {/* Sidebar Preview */}
                    <div className="ar-side" style={{ width: 220, flexShrink: 0, background: '#fff', border: '1px solid #f3f4f6', borderRadius: 18, boxShadow: '0 1px 6px rgba(0,0,0,.05)', overflow: 'hidden' }}>
                        <div style={{ padding: '24px 20px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 12, borderBottom: '1px solid #f9fafb' }}>
                            ...
                            <div>
                                ...
                                <p style={{ fontSize: 12, color: '#9ca3af', marginTop: 3 }}>
                                    {form.routeName || 'Route name will appear here'}
                                </p>
                            </div>
                    ...
                    {/* Form Card */}
                    <div className="ar-form" style={{ flex: 1, minWidth: 0, background: '#fff', border: '1px solid #f3f4f6', borderRadius: 18, boxShadow: '0 1px 6px rgba(0,0,0,.05)', padding: 32 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px 24px' }}>
                            {FIELDS.map(({ key, label, type, required }, i) => {
                                ...
                                return (
                                    <div key={key} className={`ar-f${i} fl-wrap`}>
                            ...
                            {/* Status */}
                            <div className="ar-fst" style={{ gridColumn: '1 / -1' }}>
                        ...
                        {/* Footer */}
                        <div className="ar-foot" style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid #f9fafb', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 12 }}>
                            <button onClick={() => navigate('/school/transport/routes')}
