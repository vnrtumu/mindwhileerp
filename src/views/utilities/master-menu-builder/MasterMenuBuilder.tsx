...
function getItemIds(item: MenuItem): string[] {
    if (item.children) return item.children.map((c) => c.id);
    return [item.id];
}

const loadRoleAccess = (roleId: string): Record<string, boolean> => {
    try {
    ...
    return allIds.reduce((acc, id) => ({ ...acc, [id]: defaults.includes(id) }), {} as Record<string, boolean>);
};
...
const MasterMenuBuilder = () => {
    ...
    const toggleAccess = (itemId: string) => {
        setAccess((prev) => ({ ...prev, [itemId]: !prev[itemId] }));
        setHasChanges(true);
    };

    const toggleParentItem = (item: MenuItem) => {
    ...
    const toggleSection = (section: MenuSection, enable: boolean) => {
        const ids: string[] = [];
        section.items.forEach((item) => {
    ...
    const handleReset = () => {
        ...
        const allIds = getAllMenuIds();
        const reset = allIds.reduce((acc, id) => ({ ...acc, [id]: defaults.includes(id) }), {} as Record<string, boolean>);
        setAccess(reset);
        setHasChanges(true);
    };
