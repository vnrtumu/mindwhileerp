...
import { LeadContext, LeadContextType } from 'src/context/lead-context/index';
import { LeadType } from 'src/types/lead';

const LeadFilter = () => {
    const { leads, setFilter }: LeadContextType = useContext(LeadContext);
    const newC = leads.filter((l: LeadType) => l.status === 'New' && !l.deleted).length;
    const contactedC = leads.filter((l: LeadType) => l.status === 'Contacted' && !l.deleted).length;
    const qualifiedC = leads.filter((l: LeadType) => l.status === 'Qualified' && !l.deleted).length;
    const convertedC = leads.filter((l: LeadType) => l.status === 'Converted' && !l.deleted).length;
    const lostC = leads.filter((l: LeadType) => l.status === 'Lost' && !l.deleted).length;
    const totalC = leads.filter((l: LeadType) => !l.deleted).length;
